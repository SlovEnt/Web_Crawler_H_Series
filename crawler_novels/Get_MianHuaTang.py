...
novelId = "39120"  # 长相思(全集）
ROOT_URL = "http://www.mianhuatang.la/39/{0}/".format(novelId)
...
def rtn_chapter_list_info(html):
    ...
    chapterListInfoSoup = soup.find_all(name="div", attrs={"class": "novel_list"})[0](name="dd")
...
def rtn_chapter_txt(chapterHtml):
    ...
    txtContent = soup.find_all(name="div", attrs={"class": "content"})[0].text
...
if __name__ == '__main__':
    ...
    for chapterInfo in chapterListInfo:
        ...
        print("路径：{0}，正在获取 章节：{1} 的内容！！！".format(novelFilePath, chapterInfo["text"]))
        #
