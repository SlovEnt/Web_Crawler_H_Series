...
NOVEL_URL = 'http://www.bshulou.com/book/313361.html'
...
headers = { 'Host' : 'www.bshulou.com'}
headers = { 'Referer' : 'http://www.bshulou.com/book/313361/46107445.html'}
...
txtFileName = soup.find_all(name="h1", attrs={"class": "bookTitle"})[0].text
# print(txtFileName)
...
chapterList = soup.find_all(name="div", attrs={"id": "list-chapterAll"})[0]

chapterList = chapterList.find_all(name="dd")
n=0
for chapterInfo in chapterList:
    ...
    chapterUrl = "{0}{1}".format("http://www.bshulou.com", chapterInfo.find_all(name="a")[0]['href'])
    chapterUrl2 = "{0}_2.html".format(chapterUrl[0:-5])
    print(chapterName, chapterUrl, chapterUrl2)

    for htmlUrl in [chapterUrl, chapterUrl2]:

        chapterHtml = requests.get(url=htmlUrl, params=headers)
        chapterHtml = chapterHtml.content.decode('gbk', 'ignore')
        # print(chapterHtml)

        chapterSoup = BeautifulSoup(chapterHtml, 'html.parser')

        textOne = chapterSoup.find_all(name="div", attrs={"id": "htmlContent"})[0].text
        textOne = textOne.replace("\n\r", "\n")
        textOne = textOne.replace("    ", "")
        textOne = textOne.replace("-->>本章未完，点击下一页继续阅读", "")
        textOne = textOne.replace("/>", "")
        textTwo = ""
