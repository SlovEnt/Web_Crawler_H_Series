...
novelId = "0/474"  # 长相思(全集）
ROOT_URL = "https://www.9itan.com/{0}/".format(novelId)
...
def write_txt_content(txtFileName, chapterName, chapterTxt):
    with open(txtFileName, 'a') as f:
        if "." in chapterName:
            chapterNameArr = chapterName.split(".")
            chapterName = "第{0}章 {1}".format(chapterNameArr[0], chapterNameArr[1])
        if " " in chapterName and "章" not in chapterName :
            chapterNameArr = chapterName.split(" ")
            chapterName = "第{0}章 {1}".format(chapterNameArr[0], chapterNameArr[1])
        if "章" not in chapterName :
            chapterName = "第{0}章 {1}".format("XXX", chapterName)
